      ...
      <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="bg-slate-950/50 border-b border-slate-800 px-8 py-6 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={`p-2 rounded-lg ${isAuthenticated ? 'bg-slate-800 text-slate-300' : 'bg-slate-800 text-slate-400'}`}>
          ...
          {!isAuthenticated ? (
            <div className="max-w-md mx-auto space-y-10">
              <div className="text-center space-y-4">
                <div className="inline-flex bg-slate-800 p-4 rounded-3xl mb-4">
                  <ShieldAlert className="text-slate-400 h-12 w-12" />
                </div>
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-800/50 border border-slate-700 text-white px-5 py-4 rounded-2xl focus:ring-2 focus:ring-slate-500 focus:border-transparent transition-all placeholder:text-slate-600"
                    required
                  />
                </div>
                <div className="space-y-2">
                ...
                <button 
                  disabled={isLoading}
                  className="w-full bg-slate-800 hover:bg-slate-700 text-white py-5 rounded-2xl font-black text-sm uppercase tracking-[0.2em] transition-all shadow-xl shadow-slate-950 disabled:opacity-50 flex items-center justify-center space-x-3"
                >
                  {isLoading ? (
            ...
            <div className="space-y-12 animate-in fade-in duration-700">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { label: "Engagements", value: "482", icon: <Users size={18} />, color: "text-slate-300" },
                  { label: "Market Cap Growth", value: "+22.4%", icon: <TrendingUp size={18} />, color: "text-green-500" },
                  { label: "AI Decision Sync", value: "0.4ms", icon: <Activity size={18} />, color: "text-slate-300" },
                  { label: "System Uptime", value: "99.99%", icon: <Database size={18} />, color: "text-slate-300" }
                ].map((stat, i) => (
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-slate-800/30 rounded-[2rem] border border-slate-800 p-8 space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-400 font-bold uppercase bg-slate-800 px-2 py-1 rounded">Real-time</span>
                  </div>
                    ...
                    {[
                      ...
                      <div key={i} className="flex items-center justify-between p-4 bg-slate-900/50 rounded-2xl border border-slate-800/50">
                        ...
                        <div className="text-right">
                          <p className="text-slate-300 font-black text-xs">{audit.progress}</p>
                          <div className="w-24 h-1.5 bg-slate-800 rounded-full mt-2">
                             <div className="h-full bg-slate-500 rounded-full" style={{ width: audit.progress }}></div>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div className="bg-gradient-to-br from-slate-800 to-transparent rounded-[2rem] border border-slate-800 p-8 flex flex-col justify-between">
                  <div>
                    <Sparkles className="text-slate-400 mb-6 h-8 w-8" />
                  ...
                  <button className="w-full bg-white text-slate-900 py-3 rounded-xl font-black text-xs uppercase tracking-widest mt-8 hover:bg-slate-50 transition-all">
